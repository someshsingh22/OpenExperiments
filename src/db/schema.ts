...
export const hypotheses = sqliteTable("hypotheses", {
  ...
  relatedHypothesisIds: text("related_hypothesis_ids", { mode: "json" }).notNull().$type<string[]>(),
  createdAt: integer("created_at").notNull(),
  updatedAt: integer("updated_at").notNull(),
}, (table) => [
...
export const datasetProblemStatements = sqliteTable("dataset_problem_statements", {
  datasetId: text("dataset_id").notNull().references(() => datasets.id, { onDelete: "cascade" }),
  problemStatementId: text("problem_statement_id").notNull().references(() => problemStatements.id, { onDelete: "cascade" }),
}, (table) => [
...
export const experimentResults = sqliteTable("experiment_results", {
  experimentId: text("experiment_id").primaryKey().references(() => experiments.id),
...
export const arenaVotes = sqliteTable("arena_votes", {
  id: text("id").primaryKey(),
  matchupId: text("matchup_id").notNull().references(() => arenaMatchups.id),
