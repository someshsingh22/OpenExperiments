...
-- Datasets
INSERT INTO datasets (id, name, huggingface_url, task_description, data_column_names, target_column_name, description, domain, submitted_by, created_at, updated_at) VALUES
('ds-1', 'Reddit ChangeMyView', 'https://huggingface.co/datasets/changemyview/change_my_view', 'You are a research scientist generating content-based hypotheses about whether a counterargument successfully changes the original poster''s opinion in online debates. Analyze linguistic features, argumentation strategies, and persuasive elements that predict opinion change.', '["text","author","reply_text","delta_awarded","timestamp"]', 'delta_awarded', 'A dataset of Reddit ChangeMyView posts and replies, with labels indicating whether the reply received a delta (opinion change). Contains over 30,000 discussion threads.', 'persuasion', 'system', 1726358400, 1726358400),
('ds-2', 'LaMem', 'https://huggingface.co/datasets/jmhessel/lamem', 'You are a research scientist generating content-based hypotheses about what makes an image memorable. Analyze visual features, composition, color, semantic content, and perceptual properties that contribute to memorability.', '["image","memorability_score","category","objects"]', 'memorability_score', 'Large-scale image memorability dataset with human-annotated memorability scores for diverse images. Contains 60,000 images with crowdsourced memorability annotations.', 'memorability', 'system', 1726358400, 1726358400),
('ds-3', 'Twitter Persuasion Pairs', 'https://huggingface.co/datasets/webis/webis-persuasion-16', 'You are a research scientist studying persuasive communication in short-form social media. Analyze how brevity, emotional tone, and social proof affect persuasion effectiveness in tweets.', '["tweet_text","reply_text","likes","retweets","persuasion_score"]', 'persuasion_score', 'Paired tweets and replies with crowdsourced persuasion effectiveness ratings. Covers a range of topics and conversational styles.', 'persuasion', 'system', 1726358400, 1726358400),
('ds-4', 'Visual Attention & Memorability', 'https://huggingface.co/datasets/mit/visual-attention', 'You are a research scientist studying what captures and holds visual attention. Analyze image properties that predict where people look and what they remember from natural scenes.', '["image","fixation_map","scene_category","objects"]', 'memorability_score', 'Eye-tracking and memorability data for natural scene images. Includes fixation maps, scene categories, and object annotations.', 'memorability', 'system', 1726358400, 1726358400);
...
-- Experiments (dataset_id links to platform datasets where applicable)
...
('exp-1', 'h-1', 'observational', 'completed', 'ds-1', 'Reddit ChangeMyView', 'Logistic regression on 4,000 CMV posts. Features: acknowledgment presence (LLM-annotated), argument length, citation count, response position. Covariates: topic, time of day, author karma. Bonferroni-corrected across 4 refinement steps.', 1726790400, 1726963200, 'https://osf.io/', 'system', 1726790400, 1726963200),
('exp-2', 'h-1', 'ab_test', 'completed', NULL, 'Fortune 500 Consumer Brand', 'Online A/B experiment. 50/50 traffic split between business-as-usual control and challenger implementing the hypothesized design. Primary metric: newsletter sign-ups. Two-proportion z-test.', 1727740800, 1729555200, 'https://osf.io/', 'system', 1727740800, 1729555200),
('exp-3', 'h-2', 'observational', 'completed', 'ds-1', 'Reddit ChangeMyView', 'Within-thread ranking analysis. Computed response position for each reply. Logistic regression predicting delta-award with position rank, controlling for length, time-of-day, and author history. Bonferroni correction over 4 tests.', 1728086400, 1728172800, 'https://osf.io/', 'system', 1728086400, 1728172800),
('exp-4', 'h-3', 'observational', 'completed', 'ds-2', 'LaMem', 'Paired comparison of visually similar images. LLM-annotated features: face presence, context congruence, scene category. Binomial test on preference within face-present pairs, stratified by congruence. Bonferroni correction over 3 tests.', 1731196800, 1731283200, 'https://osf.io/', 'system', 1731196800, 1731283200),
('exp-5', 'h-4', 'observational', 'completed', 'ds-1', 'Reddit ChangeMyView', 'LLM-annotated framing type (binary vs. spectrum) for 2,500 counterarguments. Logistic regression on opinion change controlling for length, acknowledgment, and citations.', 1729555200, 1729641600, 'https://osf.io/', 'system', 1729555200, 1729641600),
('exp-6', 'h-12', 'observational', 'completed', 'ds-1', 'Reddit ChangeMyView', 'Interaction analysis between accuracy-motivating elements (citations, data references) and identity-relevant topic classification. Two-way logistic regression with interaction term, controlling for length and position.', 1730246400, 1730419200, 'https://osf.io/', 'system', 1730246400, 1730419200),
('exp-7', 'h-10', 'observational', 'completed', 'ds-2', 'LaMem', 'Color palette extraction using k-means clustering on pixel values. Warm/cool classification based on dominant hue. Paired comparison within visually similar pairs, controlling for scene category and complexity.', 1733097600, 1733184000, 'https://osf.io/', 'system', 1733097600, 1733184000),
('exp-8', 'h-8', 'observational', 'completed', 'ds-1', 'Reddit ChangeMyView', 'Narrative structure annotation via LLM (personal story vs. abstract argument). Logistic regression controlling for length, position, citations, and acknowledgment.', 1732406400, 1732492800, 'https://osf.io/', 'system', 1732406400, 1732492800);
...
('exp-2', 0.000001, 3.44, 2.8, 4.1, 18500, 'The challenger produced 15,100 sign-ups versus 3,400 for control -- a +344% uplift in sign-ups. At a conservative cost-per-click of $0.22, equivalent sign-ups via paid traffic would cost ~$29,570 per week.', '+344%', 1729555200),
...
('exp-4', 0.004, 0.29, 0.12, 0.46, 1200, 'Among image pairs where both contain faces, the image with the incongruent context is preferred as more memorable 68.3% of the time (binomial p = 0.004).', NULL, 1731283200),
...
('exp-7', 0.032, 0.15, 0.01, 0.29, 800, 'Warm-palette images show a modest memorability advantage (57.2% preference rate, p = 0.032), but effect size is small and may be confounded with content type.', NULL, 1733184000),
